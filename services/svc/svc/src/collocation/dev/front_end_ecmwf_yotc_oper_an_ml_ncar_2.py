...
class front_end_ecmwf_yotc_oper_an_ml_ncar_1 (front_end_abstract_uniform):
    def load_vfile (self, filename):
        ...
        # process parameters 130 to 132
        ncfile2 = front_end_NetCDF_helper()
        ExcludeList = ['initial_time0_encoded', 'initial_time0_hours', \
                       'lv_HYBL1', 'lv_HYBL_a', 'lv_HYBL_b', \
                       'lv_HYBL_i2_a', 'lv_HYBL_i2_b', 'P0']
        
        # process parameters
        NameStrt = filename.rfind("/")
        ParamStrt = NameStrt + 34
        Suffix = ["799.nc", "1279.nc"] [GridType]

        filename2 = filename [0:ParamStrt] + "130128_132128_" + Suffix
        print "filename2:", filename2
  
        ncfile2.open (filename2)
        ncfile2.exclude (ExcludeList)

        self.load_levels  (ncfile2)
        self.load_data_tlayx (ncfile2)
