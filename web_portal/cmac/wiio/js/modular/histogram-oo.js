...
var in_place = true; /*MAKE SURE TO ADD YOUR TARGET in var pjs_target, which gets passed into onImageClick()
...
jQuery(document).ready(function(){
	...
	//else browser is not ie9...
		...
		if(!jQuery.browser.msie){
			var pjs_stretch = new pjsStretch(pjs_target);
			pjs_stretch.Init();
		}
	}
});
...
//initialize methods
var pjsStretch = function(target){
	var css_target = target;
	this.Init = function(css_target){
		this.onImageClick(target);
		this.addGreybox();
		this.hideCanvas();
	};
	this.onImageClick = function(the_target){
		...
		jQuery(the_target).each(function(i,x){
			...
			if(i == jQuery(the_target).length-1){
				return super(pjsStretch, self).imagesLoaded();
			}
			});
	...
	this.makeHistEq = function(imgobject,image_src){
	...
	this.makeHistStretch = function(imgobject,image_src){
		var imgleft = jQuery(imgobject).offset().left;
		var imgtop = jQuery(imgobject).offset().top;
		//modularImage(string),width(int),height(int) is what gets sent over to Processing.js 
		var modularImage = image_src;
